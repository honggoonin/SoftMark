...
def transformBinaryImpl(target_path, opts, metaData, granularity, reanalyze, R, hasRandSection=False, showlevel=1):
    """
    ...
    RE.performTransformation(granularity, reanalyze)
    ...
    filename = target_path.split(os.sep)[-1] + '_test.txt'

    f = open(filename, 'a')
    f.write('\n'+str(reanalyze) + ','+str(endTime - startTime)+',')
    f.close()

def transformBinary(target, opts, R, granularity=0, reanalyze=0, showlevel=1):
    ...
    if os.path.exists(shuffleInfoBin):
        transformBinaryImpl(target, opts, shuffleInfoBin, granularity, reanalyze, R, hasRandSection=False, showlevel=showlevel)
    elif os.path.exists(C.METADATA_PATH):
        transformBinaryImpl(target, opts, C.METADATA_PATH, granularity, reanalyze, R, hasRandSection=True, showlevel=showlevel)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_option("-a", "--analyze", dest="analyze", action="store_true", default=False,
                      help="Re-analyzing usable functions, even if data already exists")
    (options, args) = parser.parse_args()
    granularity = 1 if options.bbl else 0
    reanalyze = 1 if options.analyze else 0
    ...
    if isValidArgs(args):
        ...
        if options.analyze:
            ...
            ANALYZE = 'read'
        logPath= fp + '_' + ANALYZE + C.LOG_POSTFIX
        ...
        #logging.getLogger().setLevel(logging.INFO) #logging level set

        # showlevel   [1:obj, 2:fun, 3:bbl, 4:fixup]
        # granularity [0:fun, 2:bbl]
        R = report.Report()
        transformBinary(fp, opts, R, granularity=granularity, reanalyze=reanalyze, showlevel=4)
